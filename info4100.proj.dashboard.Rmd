...
author: '[[Xuyuan Liu, xl982][Jiahui, jy965][Shania Cabrera, skc93][Amparo Averbuj, aa2733][Bakhtawar Ahtisham, ba453]]'
...
The middle section is dedicated to tracking emotions, likely `` stud.moodTrend` `` and i**nst.avgMood.** Lastly, the Bottom segments are dedicated to comparison of the student's performance alongside the class average and their quiz score along a timeline of the entire course.

This layout follows good UX principles by following external standardization (ex: usage of the universal hamburger icon for a menu) and natural mapping. Natural mapping is applied by following the top-to-bottom and left-to-right reading structure in Western culture. In this case, the summary metrics are placed at the top and more detailed visualizations are placed below, with clear labeling and explanatory text.

## Teacher Dashboard Sketch

![Teacher Dashboard](teacher.jpeg)

This segment provides an overview on classroom performance by comparing undergraduate and graduate students, and course performance by year.

-   The Average skill score is used to inform the data visualization, which will report the different scores between graduate and undergraduates

-   The Quiz score section follows this same logic **`stud.avgScore`**: Average quiz score with grade icon)

-   **`stud.attendanceRate`**: Attendance percentage with calendar icon, compared by years

-   Lastly, the instructors may evaluate the average mood of students in a single class year, and may compare it with other class years
...
![General Dashboard Overview](1.jpg)

 ![Session Mood Tracker](2.jpg)
 ![Quiz Score Timeline](3.jpg)
 ![Comparison with Class Average](4.jpg)
 ![Instructor Dasboard KPIs](5.jpg)
 ![Skill Survey Score by Program](6.jpg)
 ![Quiz Score Trend by Year](7.jpg)
![Session Attendance Over Time](8.jpg)
![Session Mood Over Time](9.jpg)
![Student Quiz Performance](10.jpg)
...
-   What do you like about it?
    -   This dashboard includes an appropriate amount of features that a teacher would need. For instance, they may take a look at class averages at a single glance. The instructor may also be able to determine progress in their teaching style per year by evaluating their students' performances and moods.
-   What would you change or add to it if you had more time?
    -   It would be helpful to add more time-specific references, so instructors can easily look back at the materials they delivered on a particular day.t would be helpful to add more time-specific references, so instructors can easily look back at the materials they delivered on a particular day.
-   What was the biggest challenge you faced? How did you address it?
    -   Up to this point, our main difficulties have stemmed from challenges in collaboration.
...
-   The difficulty of this homework varied by task. Preparing the data to be ready for dashboard usage was quite straightforward. However, properly defining features for each respective dashboard took some time. It was mostly difficult to find proper collaboration times during the end of the semester when many final projects and exams coincided.
