...
-   Why? What is the goal? What questions to answer?
    ...
    -   [Spot and notify students who are falling behind early — for example, low attendance, low quiz scores, or declining temperature scores]
...
-   How? How will visualizations support the goal?
    ...
    -   [Show students positive/negative learning trend using score-over-time visualization with color-coded performance zones]
    -   [Display comparative context (e.g., class average) anonymously to help students interpret their standing without compromising peer privacy]
...
-   How? How will visualizations support the goal?
    -   [Use line charts and heatmaps to show trends across the semester in attendance, quiz performance, and mood—helping instructors identify when and where issues arise]
    -   [Use boxplots and bar charts to explore the distribution of skills and their relationship to success]
    -   [Provide interactive filters and sortable tables to easily find and support struggling students]
    -   [Group visualizations hierarchically (overview > session detail > individual student), supporting both broad and granular insights]
...
session_summary <- readRDS("session_summary.rds")
student_full_summary <- readRDS("student_full_summary.rds")
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      menuItem("Instructor Dashboard", tabName = "instructor", icon = icon("chalkboard-teacher"))
    )
  ),
  dashboardBody(
    tabItems(
      tabItem(
        ...
        # KPI Boxes
        fluidRow(
          infoBoxOutput("avgAttendance"),
          infoBoxOutput("avgScore"),
          infoBoxOutput("atRiskCount")
        ),

        # Session Trends
        fluidRow(
          box(title = "Attendance Over Time", plotOutput("attendanceTrend"), width = 6),
          box(title = "Mood (avg_t_clicker) Over Time", plotOutput("moodTrend"), width = 6)
        ),

        # Performance and Risk Analysis
        fluidRow(
          box(title = "Quiz Score Distribution by Session", plotOutput("scoreTrend"), width = 6),
          box(title = "At-Risk Students", DTOutput("riskTable"), width = 6)
        )
      )
    )
  )
...
server <- function(input, output) {
  ...
  output$avgAttendance <- renderInfoBox({
    avg_att <- mean(session_summary$session_attendance_rate, na.rm = TRUE)
    infoBox("Avg Attendance", paste0(round(avg_att * 100, 1), "%"), icon = icon("user-check"), color = "aqua")
  })

  output$avgScore <- renderInfoBox({
    avg_score <- mean(session_summary$avg_quiz_score, na.rm = TRUE)
    infoBox("Avg Quiz Score", round(avg_score, 1), icon = icon("chart-line"), color = "green")
  })

  output$atRiskCount <- renderInfoBox({
    at_risk <- student_full_summary %>% filter(student_attendance_rate < 0.7 | avg_quiz_score < 12)
    infoBox("At-Risk Students", nrow(at_risk), icon = icon("exclamation-triangle"), color = "red")
  })

  output$attendanceTrend <- renderPlot({
    ggplot(session_summary, aes(x = SESSION_NUMBER, y = session_attendance_rate)) +
      geom_line(color = "steelblue") +
      geom_point() +
      labs(title = "Attendance Rate per Session", y = "Attendance Rate", x = "Session") +
      theme_minimal()
  })

  output$moodTrend <- renderPlot({
    ggplot(session_summary, aes(x = SESSION_NUMBER, y = avg_temperature)) +
      geom_line(color = "tomato") +
      geom_point() +
      labs(title = "Average Mood per Session", y = "Avg Temperature (Mood)", x = "Session") +
      theme_minimal()
  })

  output$scoreTrend <- renderPlot({
    ggplot(session_summary, aes(x = SESSION_NUMBER, y = avg_quiz_score)) +
      geom_line(color = "darkgreen") +
      geom_point() +
      labs(title = "Quiz Score Trend", y = "Avg Quiz Score", x = "Session") +
      theme_minimal()
  })

  output$riskTable <- renderDT({
    student_full_summary %>%
      filter(student_attendance_rate < 0.7 | avg_quiz_score < 12) %>%
      select(STUDENT_KEY, student_attendance_rate, avg_quiz_score)
  })
}
